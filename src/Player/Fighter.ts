 ...
 * 格斗玩家
 */
class Fighter extends BaseModule implements IMessage
{
	public constructor() 
	{
		super();
		this.addListener(this,egret.Event.ENTER_FRAME, this.update, this);
		this.addMessage(MsgCMD.KEY_LEFTBEGIN,this);
		this.addMessage(MsgCMD.KEY_LEFTEND,this);
		this.addMessage(MsgCMD.KEY_RIGHTBEGIN,this);
		this.addMessage(MsgCMD.KEY_RIGHTEND,this);
		this.addMessage(MsgCMD.KEY_SKILL1,this);
		this.addMessage(MsgCMD.KEY_SKILL2,this);
		this.addMessage(MsgCMD.KEY_SKILL3,this);
		this.addMessage(MsgCMD.GAME_STOP,this);
		this.addMessage(MsgCMD.GAME_CONTINUE,this);
	}
	/**玩家 */
	public fighter:egret.Bitmap = null;
	/**玩家种类 */
	public fighterType:number = 0;
	/**玩家序列帧 */
	private fighterFrame:number = 1;
	/**玩家技能种类 */
	private skillType:number = 0;
	/**玩家初始化 */
	public fighterInit(type:number)
	{
	...
	public update()
	{
		...
		if(this.fighterType == 2)
		{
			if(this.frameTime % 10 == 0 && this.isSkill1 == true)
			{
				...
				this.isRelease = true;
				this.fighterFrame += 1;
				let effectX:number = this.x + this.fighter.scaleX * this.fighter.width * 0.5;
				let effectY:number = this.y - 150;
			...
			if(this.frameTime % 10 == 0 && this.isSkill2 == true)
			{
				...
				this.isRelease = true;
				this.fighterFrame += 1;
				let effectX:number = this.x + this.fighter.scaleX * this.fighter.width * 0.2;
				let effectY:number = this.y - this.fighter.height * 0.7;
			...
			if(this.frameTime % 15 == 0 && this.isSkill3 == true)
			{
				...
				this.isRelease = true;
				this.fighterFrame += 1;
				let effectX:number = this.x;
				let effectY:number = this.y - 50;
		...
		if(this.fighterType == 1)
		{
			if(this.frameTime % 10 == 0 && this.isSkill1 == true)
			{
				...
				this.fighterFrame += 1;
				let effectX:number = this.x + this.fighter.scaleX * this.fighter.width *0.5;
				let effectY:number = this.y - this.fighter.height *0.5;
				if(this.fighterFrame == 3)
				{
			...
			if(this.frameTime % 6 == 0 && this.isSkill3 == true)
			{
				...
				this.fighterFrame += 1;
				let effectX:number = this.x + this.fighter.scaleX * this.fighter.width *0.5;
				let effectY:number = this.y - this.fighter.height *0.5 + 50;
	...
	/**技能1按键按下 */
	private isSkill1:boolean = false;
	/**技能2按键按下 */
	private isSkill2:boolean = false;
	/**技能3按键按下 */
	private isSkill3:boolean = false;
